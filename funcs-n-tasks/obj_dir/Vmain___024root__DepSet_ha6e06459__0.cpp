...
VL_INLINE_OPT void Vmain___024root___nba_sequent__TOP__0(Vmain___024root* vlSelf) {
    ...
    // Body
    if (vlSelf->trigger) {
        vlSelf->count = (0x7fU & ((IData)(vlSelf->count) 
                                  + (IData)(vlSelf->inc_step)));
    }
}
...
#endif
            VL_FATAL_MT("main.v", 3, "", "NBA region did not converge.");
        }
...
#endif
                VL_FATAL_MT("main.v", 3, "", "Active region did not converge.");
            }
...
void Vmain___024root___eval_debug_assertions(Vmain___024root* vlSelf) {
    ...
    if (VL_UNLIKELY((vlSelf->start & 0x80U))) {
        Verilated::overWidthError("start");}
    if (VL_UNLIKELY((vlSelf->inc_step & 0x80U))) {
        Verilated::overWidthError("inc_step");}
